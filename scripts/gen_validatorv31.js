...
if((strlen($strdate)<10)OR(strlen($strdate)>10)){
	echo("Sisesta kuupäev 'yyyy-mm-dd' formaadis");
	}
else{
...
//The entered value is checked for proper Date format 
	if((substr_count($strdate,"-"))<>2){
		...
		if(!($result)){echo "Sisesta kehtiv kuupäev";}
	else{
		if(($date<=0)OR($date>31)){echo "Sisesta kehtiv päev";}
	}
	$month=substr($strdate,($pos+1),($pos));
	if(($month<=0)OR($month>12)){echo "Sisesta kehtiv kuu";}
	else{
		$result=ereg("^[0-9]+$",$month,$trashed);
		if(!($result)){echo "Sisesta kehtiv kuu";}
	}
	$year=substr($strdate,($pos+4),strlen($strdate));
	$result=ereg("^[0-9]+$",$year,$trashed);
	if(!($result)){echo "Sisesta kehtiv aasta";}
	else{
	if(($year<1900)OR($year>2200)){echo "Sisesta aasta vahemikus 1900-2200";}
	}
}
}
